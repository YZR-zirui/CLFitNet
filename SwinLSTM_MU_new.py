...
class PatchInflated(nn.Module):
    ...
    def __init__(self, in_chans, embed_dim, input_resolution, stride=4, padding=1, output_padding=2):
        ...
        self.Conv = nn.ConvTranspose2d(in_channels=embed_dim, out_channels=in_chans, kernel_size=(4, 4),
                                       stride=stride, padding=padding, output_padding=output_padding)
...
class SwinLSTMCell(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size, depth,
        ...
        stride = 1
        padding = 2
        self.conv_x = nn.Sequential(nn.Conv1d(in_channels=dim, out_channels=dim * 4,
                                              kernel_size=filter_size, stride=stride, padding=padding, bias=False), )
        self.conv_m = nn.Sequential(nn.Conv1d(in_channels=dim, out_channels=dim * 3,
                                              kernel_size=filter_size, stride=stride, padding=padding, bias=False), )
        self.conv_h = nn.Sequential(nn.Conv1d(in_channels=dim, out_channels=dim,
                                              kernel_size=filter_size, stride=stride, padding=padding, bias=False), )
        self.conv_o = nn.Sequential(nn.Conv1d(in_channels=dim * 2, out_channels=dim,
                                              kernel_size=filter_size, stride=stride, padding=padding, bias=False), )

        self.conv_last = nn.Sequential(nn.Conv2d(in_channels=dim * 2, out_channels=dim,
                                                 kernel_size=filter_size, stride=stride, padding=padding, bias=False))

    def forward(self, xt, hidden_states, mt):
        ...
        xt1 = xt.transpose(1, 2)
        mt1 = mt.transpose(1, 2)
        hx1 = hx.transpose(1, 2)
        x_concat = self.conv_x(xt1)
        m_concat = self.conv_m(mt1)
        h_concat = self.conv_h(hx1)

        i_x_prime, f_x_prime, g_x_prime, o_x = torch.split(x_concat, self.dim, dim=1)
        o_h = h_concat
        i_m, f_m, g_m = torch.split(m_concat, self.dim, dim=1)
        ...
        m_new = f_t_prime * mt1 + i_t_prime * g_t_prime
        cy = cy.transpose(1, 2)
        mem = torch.cat((cy, m_new), 1)

        # o_x = o_x.transpose(1, 2)
        # o_h = o_h.transpose(1, 2)
        o_t = torch.sigmoid(o_x + o_h + self.conv_o(mem))
        h_new = o_t * torch.tanh(self.conv_o(mem))

        hx = h_new.transpose(1, 2)
        mx = m_new.transpose(1, 2)
        return hx, (hx, cx), mx
...
class DownSample(nn.Module):
    ...
    def forward(self, x, y, m):
        ...
        hidden_states_down = []  # H状态
        x_downsample = []
        for index, layer in enumerate(self.layers):
            if index == self.num_layers-1:
                x, hidden_state, m = layer(x, y[index], m)  # Cell
                hidden_states_down.append(hidden_state)
            else:
                x_downsample.append(x)
                x, hidden_state, m = layer(x, y[index], m)  # Cell
                x = self.downsample[index](x)  # merging
                m = self.downsample[index](m)
                hidden_states_down.append(hidden_state)

        return hidden_states_down, x, m, x_downsample
...
class UpSample(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths_upsample, num_heads, window_size, mlp_ratio=4.,
        ...
        for i_layer in range(self.num_layers):
            concat_linear = nn.Linear(2 * int(self.embed_dim * 2 ** (self.num_layers - i_layer - 1)),
                                      int(embed_dim * 2 ** (
                                                  self.num_layers - 1 - i_layer))) if i_layer > 0 else nn.Identity()

            resolution1 = (patches_resolution[0] // (2 ** (self.num_layers - i_layer - 1)))
            resolution2 = (patches_resolution[1] // (2 ** (self.num_layers - i_layer - 1)))
            dimension = int(embed_dim * 2 ** (self.num_layers - i_layer - 1))
    ...
    def forward(self, x, y, m, x_downsample):
        hidden_states_up = []
        ...
        for index, layer in enumerate(self.layers):
            if index == 0:
                ...
                m = self.upsample[index](m)
            elif index ==self.num_layers -1:
                # print(x.size())
                x = torch.cat([x, x_downsample[3 - index]], -1)
                x = self.concat_back_dim[index](x)
                x, hidden_state, m = layer(x, y[index-1], m)
            ...
            #     x, hidden_state, m = layer(x, y[index], m)
            #     x = self.upsample[index](x)
            #     m = self.upsample[index](m)
            #     hidden_states_up.append(hidden_state)
            # else:
            ...
            # m = self.upsample[index](m)
            # print(x.size())
            # x = torch.cat([x, x_downsample[2 - index]], -1)
            # x = self.concat_back_dim[index](x)
            # x, hidden_state, m = layer(x, y[index], m)
            # x = self.upsample[index](x)
            # m = self.upsample[index](m)
...
# 1.主体内容，包含swimLSTM和 (Patch Merging) or (Patch Expanding)
# downsample是Merding部分
class SwinLSTM(nn.Module):
    ...
    def forward(self, input, states_down, states_up):
        B, C, H, W = input.shape
        memory = torch.zeros(B, C, H, W).to(input.device)
        # 下采样部分
        states_down, x, memory, x_downsample = self.Downsample(input, states_down, memory)

        # 上采样部分多一个CELL，其余不变
        states_up, output = self.Upsample(x, states_up, memory, x_downsample)
