...
# ===================== 配置 =====================
FOLDER = r"Y:\SC\sc\data\QGBZ\ScCL_QGBZ_1052_Xiang Sixiang\OD\Process\2.Initial\Label"# 存放分割图的文件夹（png/tif等都行）
OUT_CSV = r"Y:\SC\sc\data\QGBZ\ScCL_QGBZ_1052_Xiang Sixiang\OD\Process\2.Initial\wtw_results.csv"
SAVE_PREVIEW = True                      # 是否保存可视化叠加图
PREVIEW_DIR = r"Y:\SC\sc\data\QGBZ\ScCL_QGBZ_1052_Xiang Sixiang\OD\Process\2.Initial\wtw_preview"
...
TISSUE_LABEL = 153   # 角膜/眼表组织（你的模型里通常 cornea+sclera 归为这一类）
# =====================================
...
def extract_surface(seg, bg=None):
    ...
    - 旧的二值/单类分割：每列最上面的非背景像素
    - 戴巩膜镜的多类分割：优先取泪液层(TEAR_LABEL)→组织层(TISSUE_LABEL)的交界，
      这样就不会把巩膜镜本体(比如 51)当成“前表面”。
    """
    seg = seg.astype(np.uint16, copy=False)
    H, W = seg.shape
    vals = np.unique(seg)

    # 1) 多类别分割：优先找 TEAR->TISSUE 交界
    if len(vals) < 64 and (TEAR_LABEL in vals) and (TISSUE_LABEL in vals):
        ...
        # 兜底：部分列可能没有 TEAR（例如边缘/分割缺口），再尝试 BG->TISSUE
