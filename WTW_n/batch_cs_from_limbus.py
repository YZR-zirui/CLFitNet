...
# ===================== 配置（请修改） =====================
IMAGE_FOLDER = r"D:\deeplearning\WTW\seg_ak"  # 必填：分割图/原图所在文件夹（必须与 CSV 的 file 列对应）
CSV_PATH     = r"D:\deeplearning\WTW\results\limbus_wtw_results.csv"  # 必填：batch_limbus_wtw.py 导出的 CSV（含 xL_px,yL_px,xR_px,yR_px 或毫米坐标）
OUT_CSV      = r"D:\deeplearning\WTW\results\cs_1\cs.cvs"  # 必填：输出 CS 结果 CSV
SAVE_PREVIEW = True
PREVIEW_DIR  = r"D:\deeplearning\WTW\results\cs_1"  # 可留空；留空则默认 IMAGE_FOLDER/cs_preview
...
# ------------------- 坐标与矢高 -------------------
def vertical_sag_mm_from_line_and_apex(line_pts_mm):
    (x1, z1), (x2, z2) = line_pts_mm
    if np.isclose(x1, x2):
        return float(z1)
    slope = (z2 - z1) / (x2 - x1)
    z_line_at_0 = z1 + slope * (0.0 - x1)
    return float(z_line_at_0)  # apex 在原点

def perpendicular_sag_mm_from_line_and_apex(line_pts_mm):
    (x1, z1), (x2, z2) = line_pts_mm
    A = (z2 - z1)
    B = (x1 - x2)
    C = (x2*z1 - x1*z2)
...
def sagittal_depth_from_profile(x_mm, z_mm, chord_len_mm):
    """以 apex 为弦中点 & 原点；给定弦长 C 计算 vertical sag"""
    C = chord_len_mm
    xL = -C/2.0
    xR =  C/2.0
...
def process_batch(image_folder, csv_path, out_csv,
    ...
    for i, r in enumerate(rows):
        ...
        CSJ_vertical = vertical_sag_mm_from_line_and_apex(line_pts_mm)
        CSJ_perp     = perpendicular_sag_mm_from_line_and_apex(line_pts_mm)
        ...
        rec = {
            ...
            "CSJ_vertical_mm": CSJ_vertical,
            "CSJ_perpendicular_mm": CSJ_perp,
            "CS10_mm": CS10
        }
        results.append(rec)
        ...
        if save_preview:
            try:
                ...
                # 垂直矢高
                z_line_0 = vertical_sag_mm_from_line_and_apex(line_pts_mm)
                y_line_at_apex = y_apex - z_line_0/realh
                plt.plot([x_apex, x_apex], [y_apex, y_line_at_apex], 'w-', lw=1.2,
                         label=f'Vertical sag={CSJ_vertical:.3f} mm')

                Hh, Ww = img.shape[:2]
                plt.xlim(0, Ww-1); plt.ylim(Hh-1, 0)
                plt.legend(fontsize=7, loc='lower right')
                ttl = f"{fname}\nWTW={rec['WTW_mm']:.3f}  CSJ_v={CSJ_vertical:.3f}  CSJ_p={CSJ_perp:.3f}"
                if CS10 is not None and not np.isnan(CS10):
                    ttl += f"  CS10={CS10:.3f}"
                plt.title(ttl)
                plt.tight_layout()
        ...
        msg = f"[{i+1}/{len(rows)}] {fname}  WTW={rec['WTW_mm']:.3f}  CSJ_v={CSJ_vertical:.3f}  CSJ_p={CSJ_perp:.3f}"
    ...
    fieldnames = ["file","apex_x_px","apex_y_px",
                  ...
                  "WTW_mm","CSJ_vertical_mm","CSJ_perpendicular_mm","CS10_mm"]
